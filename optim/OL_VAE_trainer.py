    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for item in self.trainloader:
                ...
                recon_batch, mu, std = self.net(data)
                ...
                # 一次迭代中所有数据的误差loss，所有数据的均值mu，所有数据的方差std
        ...
        with torch.no_grad():
            for item in self.testloader:
                ...
                logger.info("index:{:.0f}\t label:{}\t prediction:{}\t mu:{:.5f}\t std:{:.5f}\t loss;{:.5f}".
                            format(int(index), label, prediction, self.test_mu, self.test_std, self.test_loss))
...
def loss_function(recon_x, x, mu, std):
    ...
    KLD = -0.5 * torch.sum(1 + 2 * torch.log(std) - mu.pow(2) - std.pow(2))
    return BCE + KLD