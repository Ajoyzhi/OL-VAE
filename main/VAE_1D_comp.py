...
def my_plot(loc: int, name: str, org_data, online_data, epoch, ):
    ...
    line1 = my_plt.plot(org_data, range(1, epoch), color='b', linestyle='*')
    line2 = my_plt.plot(online_data, range(1, epoch), color='r', linestyle='+')
    plt.xlabel('epoch')
    plt.ylabel(name)
    plt.title(name + 'of original VAE vs. online VAE')
    my_plt.legend(line1, ('original VAE'))
    my_plt.legend(line2, ('online VAE'))
    my_plt.show()

plt.figure(figsize=(12, 6))
# 画train_time的图
my_plot(151, org_train_time,ol_train_time, epoch, 'train_time')
# 画 train_loss的图
my_plot(152, org_train_loss, ol_train_loss, epoch, 'train_loss')
# 画train_mu的图
my_plot(153, org_train_mu, ol_train_mu, epoch, 'train_mu')
# 画train_var的图
my_plot(154, org_train_var, ol_train_var, epoch, 'train_var')
